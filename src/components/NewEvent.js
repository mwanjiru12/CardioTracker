...
function NewEvent({ ENDPOINT }) {
  ...
  useEffect(() => {
    authFetchActiveDays(`${ENDPOINT}/active_days/`)
      .then(json => json.filter(e => e.user_id === user.id))
      .then(j => j.length > 0 ? setActiveDaysForNewEvent(j[j.length - 1]) : setActiveDaysForNewEvent({ date: '2020-01-01' }));
    authFetchActiveDays(`${ENDPOINT}/active_days/`)
      .then(json => json.filter(e => e.user_id === user.id))
      .then(d => setMultipleActivitiesSameDay(d));
  }, []);
  ...
  function handleActivityPost(active_day) {
    ...
    authFetchSubmitNewActivity(`${ENDPOINT}/activities/`, 'POST', transformedNewEventData);
    setNewEventData({
      ...newEventData,
      exercise_type: "",
      calories: '',
  ...
  function handleMultipleActivitySameDay(newActiveDayObj) {
    ...
    const duplicateSubmittedDates = multipleActivitiesSameDay.map(d => ({
      id: d.id,
      date: new Date(d.date).getTime() + 21600000
    }));
